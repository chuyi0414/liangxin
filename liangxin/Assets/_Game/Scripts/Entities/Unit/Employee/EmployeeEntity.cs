...
// 引用 UnityEngine 命名空间，使用 SerializeField/Vector/Mathf 等类型
...
/// 功能：提供一个基础 AI（不移动）：可视范围内有敌人则在攻击范围内攻击。
/// </summary>
[RequireComponent(typeof(HybridNavigationAgent))] // 约束必须挂载导航组件（当前 AI 不主动移动，但项目单位统一要求）
[EntityPrefab("Prefabs/Entities/Unit/Employee/EmployeeEntity", "EmployeeEntity", "Employees")] // 绑定实体预制体信息
public sealed class EmployeeEntity : UnitEntity, IEntityPreShowData<EmployeePreShowData>, IEmployeeControllable // 员工实体定义（支持预显示出生点）
{
    /// <summary>
    /// 员工碰撞体到实体的映射表：用于鼠标点击快速定位员工，避免在 Update 中 GetComponent 查找。
    /// </summary>
    private static readonly Dictionary<Collider2D, EmployeeEntity> ColliderEntityMap = // 碰撞体到员工实体映射表
        new Dictionary<Collider2D, EmployeeEntity>(64); // 预分配容量以减少扩容
    ...
    /// 通过碰撞体获取员工实体（用于点击选中）。
    /// </summary>
    /// <param name="collider">员工碰撞体。</param>
    /// <param name="entity">输出员工实体。</param>
    /// <returns>是否找到。</returns>
    public static bool TryGetEntityByCollider(Collider2D collider, out EmployeeEntity entity) // 碰撞体查员工入口
    {
        entity = null; // 默认输出为空
        ...
        return ColliderEntityMap.TryGetValue(collider, out entity) && entity != null; // 返回映射结果并保护空引用
    }
