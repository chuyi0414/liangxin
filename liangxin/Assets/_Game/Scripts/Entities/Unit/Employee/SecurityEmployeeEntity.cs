...
/// <summary>
/// 保安员工实体：近战单位（不发射子弹）。
/// 说明：该脚本不继承 EmployeeEntity（按需求每个职业独立脚本），但复用 UnitEntity 的通用战斗能力。
/// </summary>
...
public sealed class SecurityEmployeeEntity : UnitEntity, IEntityPreShowData<EmployeePreShowData>, IEmployeeControllable // 保安员工实体定义
{
    /// <summary>
    /// 员工碰撞体到实体的映射表：用于鼠标点击快速定位员工，避免在 Update 中 GetComponent 查找。
    /// </summary>
    private static readonly Dictionary<Collider2D, SecurityEmployeeEntity> ColliderEntityMap = // 碰撞体到保安实体映射表
        new Dictionary<Collider2D, SecurityEmployeeEntity>(64); // 预分配容量以减少扩容
    ...
    /// </summary>
    private float _sightRange; // 保安可视范围缓存
