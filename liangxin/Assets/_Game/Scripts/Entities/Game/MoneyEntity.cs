...
public sealed class MoneyEntity : EntityBase // 金币实体定义
{
    ...
    /// <summary>是否需要在回收时结算拾取（用于控制“到达后再加钱”）。</summary>
    ...
    private void StartDropTweenFromCache() // 掉落 Tween 启动入口
    {
        ...
        _dropTween = Tween.Custom<MoneyEntity>(this, 0f, 1f, duration, (self, t) => // 创建自定义掉落动画（避免闭包捕获）
        {
            var rawProgress = Mathf.Clamp01(t); // 获取线性归一化进度（0~1）
            ...
            self.transform.position = basePosition; // 更新金币位置
    ...
    private void CommitPickupOnRecycleIfNeeded() // 拾取回收结算入口
    {
        ...
        battleDataManager.AddMoney(amount); // 在回收阶段增加资金并派发事件
    }
}
