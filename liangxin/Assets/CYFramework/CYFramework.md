...
**版本**�?.2 (Enhanced Hybrid)

**适用平台**：微信小游戏 / WebGL / Android / iOS / PC

核心愿景：构建一套“可落地”的工业级底座。在微信端追求极致轻量，�?PC 端通过 “混合架�?(Hybrid Architecture)�?—�?即用 OOP 写逻辑大脑、用 DOTS 做物理肌�?—�?来兼顾开发效率与极致性能�?

## 设计哲学与核心原�?

### 1.1 分层治之 (Layered & Decoupled)

基础设施：采�?Service Locator，实现通用模块热插拔�?

玩法核心：采�?抽象接口 (IGameplayWorld)，物理隔离逻辑与实现�?

### 1.2 性能分级与混合策�?(Tiered & Hybrid)

基线 (Baseline)：全平台默认。零 GC、对象池化、SOA 数据布局�?

增强 (Enhanced)�?*�?PC/Mobile Native �?*。采�?Hybrid DOTS 策略：复杂逻辑�?C# 写，大规模运算下放给 Job System + Burst�?

> ⚠️ **平台限制**：微信小游戏/WebGL 运行在单线程 JS 环境�?*不支�?Job System 多线�?*，只能使�?OOP Lite 实现�?

### 1.3 平台原生亲和 (Platform Native)

针对微信小游戏，直接封装 `wx.getFileSystemManager` 等底�?API，拒绝中间层损耗�?

### 1.4 数据与表现分�?(Data-View Separation)
...
> ⚠️ 本白皮书包含“架构愿�?规划”�?*能否使用、有哪些 API、是否支持某平台**，以 `Assets/CYFramework/Runtime` 的代码实现为准�?
...
| 网络 | `NetworkService` | �?已实现（HTTP/WS、重�?心跳/熔断、适配器） | `Assets/CYFramework/Runtime/Core/Network/NetworkService.cs` |
| 资源 | `IResourceLoader`/`ResourceLoader` | ⚠️ 当前实现�?Resources + 缓存；Addressables/AB 为预留接�?配置�?| `Assets/CYFramework/Runtime/Core/Resource/ResourceLoader.cs` |
...
**烘焙流程 (Build Process)**�?
- **OOP 目标**：直接拷�?打包原始 SO
- **DOTS 目标**：通过 Baker<T> 脚本，自动将 SO 数据“烘焙”为二进�?BlobAsset
...
**收益**：日常开发“改配置 -> 跑游戏”循环无等待，只有打包时才执�?Baking�?
...
- 自动重连：断线后指数退避重试（1s -> 2s -> 4s -> 8s，上�?0s�?

- 心跳保活：每 15s 发送心跳包，超�?3 次判定断�?
...
- 可�?MessagePack / Protobuf（生产环境高性能�?
...
- **类型支持**：GameObject / 纯数�?Struct / UI 节点
...
| 平台 | BGM | SFX | 特殊处理 |
|------|-----|-----|----------|
| **PC/Mobile** | Unity AudioSource | Unity AudioSource | �?|
...
2. **实例复用**：微信端禁止频繁创建 AudioContext
   ...
   - SFX：`AudioSourcePool` 预分�?8~16 个实例循环复�?
...
> ⚠️ **强制要求**：微信小游戏切后台（点胶�?接电话）时，必须静音且暂停逻辑，否�?*审核不通过**�?

```csharp
// Bootstrap �?GameManager 中注�?
void OnEnable() {
...
| **切前�?< 5s** | 正常恢复 |
| **切前�?> 5s** | 重置 deltaTime，防止角色瞬�?技�?CD 归零 |
...
不再追求“全�?ECS”，而是采用混合模式降低开发难度�?

大脑 (Brain - OOP)：复杂的技能判定、状态机、AI 决策树依然用 C# Class/Struct 编写。这部分代码可与“实�?A”复用�?

肌肉 (Muscle - DOTS)：位置更新、物理碰撞、大规模 AOE 判定、视锥剔除等“计算密集型”任务，下放�?SystemBase �?IJobEntity 中并行执行�?
...
解决 ECS 数据难以�?UI 访问的问题�?

#### 内存策略：三缓冲环形队列 (Triple Buffered Ring Queue)

**问题**：每�?`new RenderSnapshot()` 会产�?GC 压力�?
...
private int _frontIdx = 0;  // 渲染�?
private int _backIdx = 1;   // 逻辑�?
private int _idleIdx = 2;   // 备用/过渡

// Snapshot 内部数组也是预分配固定长�?
public struct RenderSnapshot {
    public int Count;                    // 实际有效数量
    public int[] IDs;                    // 预分�?MaxUnits
    public Vector3[] Positions;          // 预分�?MaxUnits
...
1. 每一�?FixedUpdate 结束后，将数据写�?Back Buffer
2. SwapBuffers() 交换索引
3. View Layer �?Front Buffer 读取，结�?Idle Buffer 做插�?

#### View Layer 消费

UI 根据 Snapshot 中的 ID 进行 Update。如�?ID 消失，则回收 UI 节点；如�?ID 新增，则�?Pool �?Spawn UI�?

**收益**：彻底解�?+ �?GC。UI 随便写，不会因为访问了被销毁的 Entity 而导�?Crash�?
...
```csharp
// 直接访问 Manager 的全�?API
CY.Entity.ShowEntity("Enemy");    // 而不�?CY.Entity.Show()
...
Assets/CYFramework/
├── Runtime/
�?  ├── CY.cs               # 统一入口（partial class，直接暴�?Manager�?
...
## 6. 关键工作�?
配置阶段：策划填�?Excel -> 导表工具生成 ScriptableObject -> 放入 Resources/Config�?

开发阶段：

绝大多数业务逻辑（技能、流程）写在 Logic_Common 中（�?C#）�?

�?Editor 模式下默认使�?Logic_OOP 运行，断点调试方便�?
...
执行 Pre-Build Baking：将 SO 转换�?DOTS BlobAssets�?

切换入口�?HybridGameplayWorld�?
...
| 技�?| 支持情况 | 替代方案 |
|------|----------|----------|
| **Job System** | �?不支�?| �?C# for 循环 + 分帧处理 |
| **Burst Compiler** | ⚠️ 有限支持 | WebGL 下自动回退�?Mono，无 SIMD |
| **NativeArray/NativeQueue** | �?不支�?| 使用普通数�?`T[]` + 对象�?|
| **Span<T>/stackalloc** | ⚠️ .NET 4.x 不支�?| 使用 `ArraySegment<T>` 或直接数组切�?|
| **System.IO 文件操作** | �?不支�?| `wx.getFileSystemManager` / IndexedDB |
| **System.Net.Sockets** | �?不支�?| 仅用 HTTP + WebSocket |
| **AppDomain** | �?不支�?| 使用 `Application.logMessageReceived`；流程系统推荐使�?Editor 生成的流程注册表替代运行时扫�?|
| **动态程序集加载** | �?不支�?| 不支�?HybridCLR，只能资源热�?|
| **原生加密�?* | ⚠️ 部分不支�?| �?C# AES 实现 �?JS 桥接 |
| **PlayerPrefs** | ⚠️ 大小受限 | 微信: `wx.setStorageSync` (上限 10MB) |
| **Addressables 本地�?* | �?不支�?| 必须使用远程 CDN 加载 |
| **多线�?async 真并�?* | �?不支�?| 单线程协程模�?|
...
| **性能测试** | Tick 耗时 / GC / 内存 | Unity Profiler + 自定�?Benchmark |

### 10.2 Mock 策略

- 所有平台适配器通过接口注入，测试时替换�?Mock 实现
- 网络层支持本�?Mock Server 模式
- 存档系统支持内存存储 Mock
